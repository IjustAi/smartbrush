...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA (GPU) is not available. This code requires a GPU to run.")
    ...
    levels = [41, 241, 321, 401]
    folder_path_1 = './SegTrackv2/JPEGImages'
    folder_path_2 = './SegTrackv2/GroundTruth'
    dataset = []
    count = 0

    for subfolder in tqdm(os.listdir(folder_path_1)):
        label = subfolder
        subfolder_path_1 = os.path.join(folder_path_1, subfolder)
        print(subfolder_path_1)

        # 把text embedding放在循环外，减少内存使用
        CLIP_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch16")
        CLIP_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch16")

        if os.path.isdir(subfolder_path_1):
            for file_name in tqdm(os.listdir(subfolder_path_1)):
                ...
                if os.path.exists(subfolder_path_2):
                    ...
                    for ground_truth_file_path in ground_truth_files:
                        ...
                        if file_basename_2 == file_basename_1:
                            ...
                            sub_dataset = MaskedImageDataset(file_path_1, ground_truth_file_path, levels, label,
                                                             batch_size, image_size, device, CLIP_processor, CLIP_model)
                            count += 1
